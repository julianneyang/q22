...
here::i_am("Q22_Rproj/Q22_L2_Maaslin2.R")
...
counts <- read.delim("Q22_Microbiome/collapsed_ASV/export_L2_Q22_ASV/feature-table.tsv", header = TRUE, row.names=1)
...
lc_phyla <- lumcol_counts

ileum_phyla <- ileum_counts
...
run_Maaslin2 <- function(counts_filepath, metadata_filepath, subset_string, fixed_effects_vector, results_string, random_effects_vector) {
  ...
  readr::write_rds(Relative_Abundance,paste0("Q22_Microbiome/differential_taxa/Relative_Abundance-",subset_string,"-L2.RDS"))
...
run_Maaslin2(lc_phyla,lumcol_meta,"Colon", fixed_effects, "_L2_Maaslin2_Sex_Q22_1-Litter", random_effects)

# Ileum
fixed_effects <- c("Sex", "Q22")
random_effects <- c("Litter")
run_Maaslin2(ileum_phyla,ileum_meta,"Ileum", fixed_effects, "_L2_Maaslin2_Sex_Q22_1-Litter", random_effects)

# Luminal Colon: 0 significant features by Genotype 

data<-read.table("Q22_Microbiome/differential_taxa/Colon_L2_Maaslin2_Sex_Q22_1-Litter/all_results.tsv", header=TRUE)
data <- data %>% filter(qval <0.25) %>% filter(metadata=="Q22")

# Ileum: 0 

data<-read.table("Q22_Microbiome/differential_taxa/Ileum_L2_Maaslin2_Sex_Q22_1-Litter/all_results.tsv", header=TRUE)
data <- data %>% filter(qval <0.25) %>% filter(metadata=="Q22")
